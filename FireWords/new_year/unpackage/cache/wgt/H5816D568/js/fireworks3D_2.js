...
// 	animate();
// }
function CreateFireObj(x, y, color, offsetValueX, offsetValueY, ctx) {
	this.initCnt = Math.random() * 20;//计数器
	this.fragArr = [];
	this.initialX = x;
	this.initialY = y
	this.x = x;
	...
	this.boom = function () {
		...
		for (var i = 0; i < scope; i++) {
			...
			var color = 'rgb(' + r + ',' + g + ',' + b + ')';
			var frag = new CreateFrag(this.x, this.y, color, targetX, targetY, ctx);
			this.fragArr.push(frag);
		}
	}
}
function CreateFrag(x, y, color, tx, ty, ctx) {
	...
	that.move = function () {
		...
		let time = 100 * Math.random() + 100;
		if(that.initCnt>time){
			that.initCnt = 0;
			that.fragDisappear = true;
		}
	}
...
// 	var color = 'rgb(' + r + ',' + g + ',' + b + ')';
// 	var fire = new func(960 + getRandom(-300, 300), 800, color, getRandom(-5, 5), getRandom(0, 3), ctx);
// 	return fire;
// }
